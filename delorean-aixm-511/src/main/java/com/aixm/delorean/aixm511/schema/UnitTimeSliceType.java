...
public class UnitTimeSliceType extends AbstractAIXMTimeSliceType implements Serializable {
     ...
     *
     */
    @ManyToMany(targetEntity = UnitDependencyPropertyType.class, cascade = {CascadeType.ALL}, fetch = FetchType.EAGER)
    @JoinTable(name = "relatedunit_unit_link", schema = "organisation", joinColumns = {
            @JoinColumn(name = "relatedunit", referencedColumnName = "hjid")}, inverseJoinColumns = {
                    @JoinColumn(name = "unitpropertygroup", referencedColumnName = "hjid")})
     ...
     *
     */
    @ManyToMany(targetEntity = UnitAvailabilityPropertyType.class, cascade = {CascadeType.ALL}, fetch = FetchType.EAGER)
    ...
    @JoinColumn(name = "EXTENSION_UNIT_TIME_SLICE_TY_0")
