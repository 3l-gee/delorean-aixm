package com.aixm.delorean.aixm52.engine;
...
public class Aixm52Engine extends com.aixm.delorean.core.engine.AbstractEngine<AIXMBasicMessageType, AbstractAIXMFeatureType, AbstractAIXMTimeSliceType, AbstractAIXMObjectType> {
    ...
    public void info(AIXMBasicMessageType message) {
        ...
        for (BasicMessageMemberAIXMPropertyType member : message.getHasMember()) { 
            ...
            if (feature != null) {
                TemporalityInspector inspector = Aixm52TimeSliceEngine.getTimeSliceValidityPeriod(feature);
    ...
    public String statistics(AIXMBasicMessageType message) {
        ...
        for (BasicMessageMemberAIXMPropertyType member : message.getHasMember()) {
            ...
            if (feature != null) {
                featureCount += 1;
                timeSliceCount += Aixm52TimeSliceEngine.countTimeSlices(feature);
            }
    ...
    private <FTYPE extends AbstractAIXMFeatureType> FTYPE integrateAixmFeature(Class<FTYPE> featureType, AbstractAIXMFeatureType currentFeature, AbstractAIXMFeatureType newPartialFeature) {
        ...
        List<AbstractAIXMTimeSliceType> listNewPartialTimeSlice = Aixm52TimeSliceEngine.invokeTimeSlice(newPartialFeature);
        List<AbstractAIXMTimeSliceType> listCurrentTimeSlice = Aixm52TimeSliceEngine.invokeTimeSlice(currentFeature);
    ...
    /**
     * Creates a delta AIXM time slice by comparing old and new objects and preserving differences, converting them into a PERMDELTA time slice. The method uses reflection to access fields and determine differences, handling both JAXBElement and List types appropriately.
     * 
     * @param <CT> the type of the AIXM time slice, extending AbstractAIXMTimeSliceType
     * @param type the class type of the AIXM time slice to instantiate
     * @param oldObj the previous/original AIXM time slice object
     * @param newObj the updated AIXM time slice object
     * @return a new time slice instance of type CT containing the delta changes
     * @throws RuntimeException if instantiation fails, field access is denied, or an unexpected field type is encountered
     */
